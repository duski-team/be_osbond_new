...
export class Controller {
    ...
    static async register(req, res) {
        const { nama_berita, status_berita, konten_berita } = req.body
        ...
        try {
            ...
            let data = await berita_m.create({ id: nanoid(14), nama_berita, status_berita, konten_berita, gambar_berita: f1 })
    ...
    static async update(req, res) {
        const { id, nama_berita, status_berita, konten_berita } = req.body
        ...
        try {
            await sq.transaction(async t => {
                ...
                let data = await berita_m.update({ nama_berita, status_berita, konten_berita }, { where: { id }, transaction: t })
    ...
    static async list(req, res) {
        const { halaman, jumlah, nama_berita, status_berita } = req.body
        ...
        try {
            ...
            where b."deletedAt" isnull ${isi} order by b."createdAt" desc ${isi2}`,
                tipe({ nama_berita: `%${nama_berita}%`, status_berita, offset: (+halaman * jumlah), jumlah: jumlah }))
            if (halaman && jumlah) {
                let jml = await sq.query(`select count(*) as "total" from berita b
                where b."deletedAt" isnull ${isi}`, tipe({ nama_berita: `%${nama_berita}%`, status_berita }))
